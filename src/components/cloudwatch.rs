...
use crate::event_managment::event::{AWSServiceNavigatorEvent, ComponentActions, Event, InputBoxEvent, TabEvent, WidgetActions, WidgetEventType, WidgetType, CloudWatchComponentActions};
...
impl CloudWatch {
    ...
    async fn handle_log_group_selection(&mut self, log_group: String) {
        ...
        self.base.navigator.set_title(format!("Log Group: {}", log_group));
        ...
        if let Some(client) = &self.cloudwatch_client {
            ...
            self.base.results_navigator.set_title(String::from("Log Events"));
            self.base.results_navigator.set_content(NavigatorContent::Records(logs));
        }
    }
    ...
    async fn search_logs(&mut self, log_group: &str, filter_pattern: &str) {
        if let Some(client) = &self.cloudwatch_client {
            ...
            self.base.results_navigator.set_title(format!("Search Results: {}", filter_pattern));
            self.base.results_navigator.set_content(NavigatorContent::Records(logs));
        }
    }
    
    async fn view_log_details(&mut self, log_content: &str) {
        self.base.details_popup.set_profile_list(PopupContent::Details(log_content.to_string()));
...
impl AWSComponent for CloudWatch {
            ...
            if let Some(signal) = self.base.details_popup.handle_input(key_event) {
                self.base.event_sender
                    .send(Event::Tab(TabEvent::ComponentActions(
                        ComponentActions::CloudWatchComponentActions(CloudWatchComponentActions::WidgetActions(signal))
                    )))
                    .unwrap();
                return;
            }
        ...
        match event {
            ComponentActions::CloudWatchComponentActions(cw_event) => match cw_event {
                ...
                CloudWatchComponentActions::PopupDetails(details) => {
                    self.base.details_popup.set_profile_list(PopupContent::Details(details.clone()));
                    self.base.details_popup.set_visible(true);
                    self.base.details_popup.set_active(true);
                }
                                ...
                                match signal {
                                    WidgetActions::AWSServiceNavigatorEvent(
                                        ...
                                        self.base.event_sender
                                            .send(Event::Tab(TabEvent::ComponentActions(
                                                ComponentActions::CloudWatchComponentActions(
                                                    CloudWatchComponentActions::PopupDetails(log_content)
                                                ),
                                            )))
                                            .unwrap();
                                    }
    ...
    async fn update(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        if let Some(client) = &self.cloudwatch_client {
            ...
            self.base.navigator.set_content(NavigatorContent::Records(log_groups));
